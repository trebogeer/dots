...
func main() {
    ...
    host := flag.String("host", "greenapi301p.prod.ch3.s.com", "MongoS host to connect to.")
    port := flag.Int("port", 20000, "MongoS port to connect to.")
    user := flag.String("u","DBMON", "MongoDB username.")
    pwd := flag.String("p", "ch3ck1ng", "MongoDB password.")
    ...
    err = chunks_c.Find(bson.M{"ns":ns}).All(&chunks)
    if err != nil {
...
func findOrphans(waitGroup *sync.WaitGroup ,db, c, u, p string, shard *Shard, chunks []Chunk) {
 ...
 // target := session.DB(db).C(c + "_orphans")
    ...
    it := tq.Iter()
//    fmt.Println("[" + shard.Host + "] Find and iterator.")
    cnt := 0
    ch_l := len(chunks)
